...
public final class FindBreakpointEvidenceSpark extends GATKSparkTool {
    ...
    @VisibleForTesting static List<SVInterval> getIntervals(
        ...
        if ( locations.evidenceDir != null ) {
            evidenceRDD
                    .filter(ev -> ev instanceof ReadEvidence)
                    .map(ev -> (ReadEvidence)ev)
                    .filter(ReadEvidence::isValidated).saveAsTextFile(locations.evidenceDir);
        }

        List<BreakpointEvidence> allEvidence =
                collectAllEvidence(evidenceRDD,contigs.size(),broadcastMetadata,
                        minEvidenceWeight,minCoherentEvidenceWeight);
        ...
        // recluster the new evidence -- we can now glue across partition boundaries
        final int maxFragmentSize = broadcastMetadata.value().getMaxMedianFragmentSize();
        Iterator<BreakpointEvidence> evidenceIterator =
            new FlatMapGluer<>(new BreakpointEvidenceClusterer(maxFragmentSize,null),
                               allEvidence.iterator(),
                               new BreakpointEvidence(new SVInterval(contigs.size(),1,1),0));
                ...
                evidenceRDD
                        .mapPartitionsWithIndex( (idx, readEvidenceItr) ->
                                new FlatMapGluer<>(
                                        ...
                                        readEvidenceItr,
                                        new BreakpointEvidence(new SVInterval(nContigs,1,1),0)), true)
                        .collect();

        // reapply the density filter (all data collected -- no more worry about partition boundaries).
        Iterator<BreakpointEvidence> evidenceIterator =
                new BreakpointDensityFilter(0, allEvidence.iterator(), null,
                        minEvidenceWeight, minCoherentEvidenceWeight);
        List<BreakpointEvidence> allEvidence2 = new ArrayList<>(allEvidence.size());
        while ( evidenceIterator.hasNext() ) {
            allEvidence2.add(evidenceIterator.next());
        }
        return allEvidence2;
    }
